...
void main() {
  group('NotificationService', () {
    ...
    group('initialize', () {
      ...
      test(
        'should throw NotificationInitializationException when initialization fails',
        () async {
          // Arrange
          when(
            mockNotifications.initialize(
      ...
      test(
        'should continue initialization when permission denied (non-fatal)',
        () async {
          // Arrange
          when(
            mockNotifications.initialize(
    ...
    group('scheduleDailyWeatherNotification', () {
      ...
      test('should schedule daily notification successfully', () async {
        ...
        expect(
          true,
          true,
          reason: 'Daily notification scheduling completed without errors',
        );
      });

      test(
        'should throw NotificationSchedulingException when location not set',
        () async {
          // Arrange
          ...
          expect(
            ...
            throwsA(
              isA<NotificationSchedulingException>().having(
                (e) => e.message,
                'message',
                contains('No location set'),
              ),
            ),
            reason:
                'Should throw NotificationSchedulingException when location not set',
      ...
      test(
        'should throw NotificationSchedulingException when announcement time not set',
        () async {
          ...
          expect(
            ...
            throwsA(
              isA<NotificationSchedulingException>().having(
                (e) => e.message,
                'message',
                contains('Announcement time not set'),
              ),
            ),
            reason:
                'Should throw NotificationSchedulingException when time not set',
          );
        },
      );
      ...
      test('should handle scheduling errors gracefully', () async {
        ...
        expect(
          ...
          reason:
              'Should wrap scheduling errors in NotificationSchedulingException',
        );
      });
    });
    ...
    group('Recurring Notifications', () {
      ...
      test(
        'should schedule daily recurring notifications when isRecurring is true',
        () async {
          // Arrange
          ...
          expect(
            true,
            true,
            reason: 'Daily recurring notifications scheduled successfully',
          );
        },
      );
      ...
      test('should schedule weekdays only recurring notifications', () async {
        ...
        expect(
          true,
          true,
          reason: 'Weekdays recurring notifications scheduled successfully',
        );
      });
      ...
      test('should schedule weekend only recurring notifications', () async {
        ...
        expect(
          true,
          true,
          reason: 'Weekend recurring notifications scheduled successfully',
        );
      });
      ...
      test('should schedule custom days recurring notifications', () async {
        ...
        expect(
          true,
          true,
          reason: 'Custom recurring notifications scheduled successfully',
        );
      });

      test(
        'should fallback to single notification when isRecurring is false',
        () async {
          // Arrange
          when(mockSettingsService.isRecurring).thenReturn(false);
          ...
          expect(
            true,
            true,
            reason: 'Single notification scheduled when recurring is disabled',
          );
        },
      );

      test(
        'should use settings service values when parameters are not provided',
        () async {
          ...
          expect(
            ...
            true,
            reason:
                'Settings service values used correctly for recurring notifications',
          );
        },
      );

      test(
        'should handle weather service errors gracefully in recurring mode',
        () async {
          // Arrange
          when(
            mockWeatherService.getWeatherByLocation(any),
          ...
          expect(
            ...
            true,
            reason:
                'Recurring notifications scheduled despite weather API errors',
          );
        },
      );

      test(
        'should throw NotificationSchedulingException when recurring setup fails',
        () async {
          // Arrange
          when(mockSettingsService.isRecurring).thenReturn(true);
          when(
          ...
          expect(
            ...
            throwsA(isA<NotificationSchedulingException>()),
            reason:
                'Should wrap recurring scheduling errors in NotificationSchedulingException',
          );
        },
      );
    });
