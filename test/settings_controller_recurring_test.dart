...
void main() {
  ...
  group('SettingsController Recurring Functionality', () {
    ...
    test('loads recurring settings from service on init', () {
      ...
      when(mockBox.get('isRecurringPaused')).thenReturn(false);
      when(
        mockBox.get('recurrencePattern'),
      ).thenReturn(RecurrencePattern.weekdays.index);
      ...
      expect(
        ...
        true,
        reason: 'Should load recurring flag from settings service',
      );
      expect(
        controller.recurrencePattern,
        RecurrencePattern.weekdays,
        reason: 'Should load recurrence pattern from settings service',
      );
      expect(
        controller.recurrenceDays,
        [1, 2, 3, 4, 5],
        reason: 'Should load recurrence days from settings service',
      );
    });

    test(
      'toggleRecurring updates settings service and reactive state',
      () async {
        // Arrange
        when(
          mockBox.put('isRecurring', true),
        ).thenAnswer((_) async => Future.value());
        when(
          mockBox.get('recurrencePattern'),
        ).thenReturn(RecurrencePattern.daily.index);
        ...
        expect(
          controller.isRecurring,
          true,
          reason: 'Reactive state should update after toggle',
        );
      },
    );

    test(
      'updateRecurrencePattern updates pattern and days for non-custom patterns',
      () async {
        // Arrange
        const pattern = RecurrencePattern.weekends;
        when(
          mockBox.put('recurrencePattern', pattern.index),
        ).thenAnswer((_) async => Future.value());
        when(
          mockBox.put('recurrenceDays', pattern.defaultDays),
        ).thenAnswer((_) async => Future.value());
        ...
        expect(
          controller.recurrencePattern,
          pattern,
          reason: 'Pattern should be updated',
        );
        expect(
          controller.recurrenceDays,
          [6, 7],
          reason: 'Days should be updated to pattern defaults for weekends',
        );
      },
    );

    test(
      'updateRecurrencePattern does not auto-update days for custom pattern',
      () async {
        // Arrange
        ...
        expect(
          controller.recurrencePattern,
          pattern,
          reason: 'Pattern should be updated to custom',
        );
      },
    );
    ...
    test('toggleRecurrenceDay adds day when not present', () async {
      ...
      controller.onInit();
      when(
        mockBox.put('recurrenceDays', [1, 2, 3, 4, 5, 6]),
      ).thenAnswer((_) async => Future.value());
      ...
      expect(
        controller.recurrenceDays,
        contains(6),
        reason: 'Should add Saturday to the list',
      );
    });
    ...
    test('toggleRecurrenceDay removes day when present', () async {
      ...
      controller.onInit();
      when(
        mockBox.put('recurrenceDays', [1, 2, 3, 4, 5, 7]),
      ).thenAnswer((_) async => Future.value());
      ...
      expect(
        controller.recurrenceDays,
        isNot(contains(6)),
        reason: 'Should remove Saturday from the list',
      );
    });
    ...
    test('getFullDayName returns correct full day names', () {
      expect(
        controller.getFullDayName(1),
        'Monday',
        reason: '1 should map to Monday',
      );
      expect(
        controller.getFullDayName(2),
        'Tuesday',
        reason: '2 should map to Tuesday',
      );
      expect(
        controller.getFullDayName(3),
        'Wednesday',
        reason: '3 should map to Wednesday',
      );
      expect(
        controller.getFullDayName(4),
        'Thursday',
        reason: '4 should map to Thursday',
      );
      expect(
        controller.getFullDayName(5),
        'Friday',
        reason: '5 should map to Friday',
      );
      expect(
        controller.getFullDayName(6),
        'Saturday',
        reason: '6 should map to Saturday',
      );
      expect(
        controller.getFullDayName(7),
        'Sunday',
        reason: '7 should map to Sunday',
      );
    });
  });
}
