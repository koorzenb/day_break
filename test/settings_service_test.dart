...
void main() {
  ...
  group('SettingsService', () {
    ...
    test('setAnnouncementHour saves hour to the box', () async {
      const hour = 7;
      when(
        mockBox.put('announcementHour', hour),
      ).thenAnswer((_) async => Future.value());
    ...
    test('setAnnouncementMinute saves minute to the box', () async {
      const minute = 30;
      when(
        mockBox.put('announcementMinute', minute),
      ).thenAnswer((_) async => Future.value());
    ...
    group('Recurring Settings', () {
      test('isRecurring defaults to false when not set', () {
        ...
        expect(
          settingsService.isRecurring,
          false,
          reason: 'isRecurring should default to false when not set',
        );
      });
      ...
      test('setIsRecurring saves recurring flag to the box', () async {
        ...
        when(
          mockBox.put('isRecurring', isRecurring),
        ).thenAnswer((_) async => Future.value());
        ...
        expect(
          settingsService.isRecurring,
          isRecurring,
          reason: 'isRecurring should return the saved value',
        );
      });
      ...
      test('recurrencePattern defaults to daily when not set', () {
        ...
        expect(
          settingsService.recurrencePattern,
          RecurrencePattern.daily,
          reason: 'recurrencePattern should default to daily when not set',
        );
      });
      ...
      test('setRecurrencePattern saves pattern index to the box', () async {
        ...
        expect(
          settingsService.recurrencePattern,
          pattern,
          reason: 'recurrencePattern should return the saved pattern',
        );
      });
      ...
      test('recurrencePattern handles invalid index gracefully', () {
        ...
        expect(
          settingsService.recurrencePattern,
          RecurrencePattern.daily,
          reason: 'Invalid pattern index should fallback to daily',
        );
      });
      ...
      test('recurrenceDays returns pattern defaults when not set', () {
        ...
        expect(
          settingsService.recurrenceDays,
          [1, 2, 3, 4, 5],
          reason: 'recurrenceDays should return weekdays default when not set',
        );
      });
      ...
      test('setRecurrenceDays saves custom days to the box', () async {
        ...
        when(
          mockBox.put('recurrenceDays', customDays),
        ).thenAnswer((_) async => Future.value());
        ...
        expect(
          settingsService.recurrenceDays,
          customDays,
          reason: 'recurrenceDays should return the saved custom days',
        );
      });
      ...
      test('setRecurringConfig sets all recurring settings at once', () async {
        ...
        await settingsService.setRecurringConfig(
      ...
      test(
        'setRecurringConfig uses pattern defaults when no custom days provided',
        () async {
          const isRecurring = true;
