...
@GenerateNiceMocks([MockSpec<WeatherService>(), MockSpec<SettingsService>(), MockSpec<FlutterLocalNotificationsPlugin>()])
void main() {
  group('NotificationService Background Scheduling Renewal Tests', () {
    ...
    group('_maintainRecurringSchedule', () {
      ...
      test('should extend schedule when fewer than 7 notifications are pending', () async {
        // Arrange
        ...
        when(mockSettingsService.recurrenceDays).thenReturn([1, 2, 3, 4, 5, 6, 7]); // Daily
        when(mockSettingsService.announcementHour).thenReturn(8);
        when(mockSettingsService.announcementMinute).thenReturn(0);
        when(mockSettingsService.location).thenReturn('Halifax, NS');
        ...
        when(mockNotifications.pendingNotificationRequests()).thenAnswer((_) async => mockPendingNotifications);
        when(mockNotifications.cancelAll()).thenAnswer((_) async {});
        when(
          mockNotifications.zonedSchedule(
            any,
        ...
        ).called(greaterThan(7)); // At least 7 for the first 7 days, plus potentially more
      });

      test('should not extend schedule when 7 or more notifications are pending', () async {
        // Arrange
        when(mockSettingsService.isRecurring).thenReturn(true);
        when(mockSettingsService.recurrencePattern).thenReturn(RecurrencePattern.daily);
        when(mockSettingsService.recurrenceDays).thenReturn([1, 2, 3, 4, 5, 6, 7]); // Daily
        ...
        final mockPendingNotifications = List.generate(8, (i) => PendingNotificationRequest(i, 'Test $i', 'Body $i', ''));
        when(mockNotifications.pendingNotificationRequests()).thenAnswer((_) async => mockPendingNotifications);
        when(mockNotifications.cancelAll()).thenAnswer((_) async {});
        when(
          mockNotifications.zonedSchedule(
    ...
    group('handleSettingsChange', () {
      test('should cancel all notifications and reschedule with new settings', () async {
        // Arrange
        when(mockSettingsService.isRecurring).thenReturn(true);
        when(mockSettingsService.recurrencePattern).thenReturn(RecurrencePattern.weekdays);
        when(mockSettingsService.recurrenceDays).thenReturn([1, 2, 3, 4, 5]); // Weekdays
      ...
      test('should handle single notification scheduling when recurring is disabled', () async {
        // Arrange
        when(mockSettingsService.isRecurring).thenReturn(false);
    ...
    group('_extendRecurringSchedule', () {
      ...
      test('should handle custom recurrence pattern correctly', () async {
        ...
        when(mockSettingsService.recurrenceDays).thenReturn([1, 3, 5]); // Monday, Wednesday, Friday
        when(mockSettingsService.announcementHour).thenReturn(8);
        when(mockSettingsService.announcementMinute).thenReturn(30);
        when(mockSettingsService.location).thenReturn('Calgary, AB');

        final mockPendingNotifications = [const PendingNotificationRequest(0, 'Test', 'Body', '')];
        when(mockNotifications.pendingNotificationRequests()).thenAnswer((_) async => mockPendingNotifications);
        when(mockNotifications.cancelAll()).thenAnswer((_) async {});
        when(
          mockNotifications.zonedSchedule(
    ...
    group('Error Handling', () {
      test('should handle errors in _maintainRecurringSchedule gracefully', () async {
        // Arrange
        when(mockSettingsService.isRecurring).thenReturn(true);
        when(mockSettingsService.recurrencePattern).thenReturn(RecurrencePattern.daily);
        ...
        when(mockSettingsService.location).thenReturn('Halifax, NS');
        when(mockNotifications.cancelAll()).thenAnswer((_) async {});
        when(mockNotifications.pendingNotificationRequests()).thenThrow(Exception('Notification error'));
      ...
      test('should handle errors in handleSettingsChange properly', () async {
        // Arrange
        when(mockSettingsService.isRecurring).thenReturn(true);
        when(mockNotifications.cancelAll()).thenThrow(Exception('Cancel error'));

        // Act & Assert
        expect(
          () async => await notificationService.handleSettingsChange(),
          throwsA(isA<Exception>()),
          reason: 'handleSettingsChange should propagate errors to caller for proper error handling in UI',
        );
      });
      ...
      test('should handle missing settings gracefully during extension', () async {
        // Arrange
        when(mockSettingsService.isRecurring).thenReturn(true);
        when(mockSettingsService.announcementHour).thenReturn(null); // Missing hour
        when(mockSettingsService.announcementMinute).thenReturn(null); // Missing minute
        ...
        expect(
          () async => await notificationService.scheduleDailyWeatherNotification(),
          throwsA(isA<NotificationException>()),
          reason: 'Should throw NotificationException when announcement time is missing',
        );
      });
    });
  });
