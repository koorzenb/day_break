...
void main() {
  group('LocationService', () {
    ...
    test(
      'determinePosition returns position when location services are enabled and permission is granted',
      () async {
        when(
    ...
    test(
      'determinePosition throws LocationServicesDisabledException when services are disabled',
      () async {
        when(
          mockGeolocator.isLocationServiceEnabled(),
    ...
    test(
      'determinePosition requests permission when denied and then returns position',
      () async {
        when(
          mockGeolocator.isLocationServiceEnabled(),
        ).thenAnswer((_) async => true);
    ...
    test(
      'determinePosition throws LocationPermissionDeniedException when permission is denied twice',
      () async {
        when(
          mockGeolocator.isLocationServiceEnabled(),
        ).thenAnswer((_) async => true);
    ...
    group('getCurrentLocationSuggestion', () {
      test(
        'throws LocationServicesDisabledException when location services are disabled',
        () async {
          when(
            mockGeolocator.isLocationServiceEnabled(),
          ).thenAnswer((_) async => false);
          ...
          expect(
            locationService.getCurrentLocationSuggestion(),
            throwsA(isA<LocationServicesDisabledException>()),
          );
        },
      );
      ...
      test(
        'throws LocationPermissionDeniedException when permission is denied',
        () async {
          when(
            mockGeolocator.isLocationServiceEnabled(),
          ).thenAnswer((_) async => true);
      ...
      test(
        'throws LocationPermissionPermanentlyDeniedException when permission is denied forever',
        () async {
          when(
            mockGeolocator.isLocationServiceEnabled(),
          ).thenAnswer((_) async => true);
