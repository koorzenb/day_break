...
void main() {
  ...
  group('AnnouncementScheduler Public API', () {
    ...
    test('NotificationConfig has sensible defaults', () {
      ...
      expect(config.channelId, 'scheduled_announcements',
          reason: 'Default channel ID should be set');
      expect(config.channelName, 'Scheduled Announcements',
      ...
      expect(config.showBadge, true,
          reason: 'Default should show badge');
      expect(config.enableLights, true,
          reason: 'Default should enable lights');
    ...
    test('RecurrencePattern has display names', () {
      expect(RecurrencePattern.daily.displayName, 'Daily',
          reason: 'Daily should have display name');
      expect(RecurrencePattern.weekdays.displayName, 'Weekdays',
          reason: 'Weekdays should have display name');
      expect(RecurrencePattern.weekends.displayName, 'Weekends',
          reason: 'Weekends should have display name');
      expect(RecurrencePattern.custom.displayName, 'Custom',
    ...
    test('ScheduledAnnouncement returns effective days correctly', () {
      ...
      expect(daily.effectiveDays, [1, 2, 3, 4, 5, 6, 7],
          reason: 'Daily should return all days');
      expect(custom.effectiveDays, [1, 3, 5],
          reason: 'Custom should return specified days');
      expect(oneTime.effectiveDays, isEmpty,
          reason: 'One-time should return empty list');
    });

    test('AnnouncementStatus has display names', () {
      expect(AnnouncementStatus.scheduled.displayName, 'Scheduled',
          reason: 'Scheduled status should have display name');
      expect(AnnouncementStatus.delivering.displayName, 'Delivering',
          reason: 'Delivering status should have display name');
      expect(AnnouncementStatus.completed.displayName, 'Completed',
          reason: 'Completed status should have display name');
      expect(AnnouncementStatus.failed.displayName, 'Failed',
          reason: 'Failed status should have display name');
    });

    test('AnnouncementStatus identifies active vs complete states', () {
      expect(AnnouncementStatus.scheduled.isActive, true,
          reason: 'Scheduled should be active');
      expect(AnnouncementStatus.delivering.isActive, true,
          reason: 'Delivering should be active');
      expect(AnnouncementStatus.completed.isActive, false,
          reason: 'Completed should not be active');
      expect(AnnouncementStatus.failed.isActive, false,
          reason: 'Failed should not be active');

      expect(AnnouncementStatus.scheduled.isComplete, false,
          reason: 'Scheduled should not be complete');
      expect(AnnouncementStatus.delivering.isComplete, false,
          reason: 'Delivering should not be complete');
      expect(AnnouncementStatus.completed.isComplete, true,
          reason: 'Completed should be complete');
      expect(AnnouncementStatus.failed.isComplete, true,
    ...
    test('Exceptions have appropriate messages', () {
      ...
      expect(permissionDenied.message, contains('permission denied'),
      ...
      expect(initError.message, 'Init failed',
      ...
      expect(ttsInit.message, 'TTS unavailable',
          reason: 'Should preserve TTS error message');

      const ttsAnnounce = TTSAnnouncementException('Speech failed');
      expect(ttsAnnounce.message, 'Speech failed',
          reason: 'Should preserve TTS announcement error message');

      const scheduling =
          NotificationSchedulingException('Scheduling failed');
      expect(scheduling.message, 'Scheduling failed',
          reason: 'Should preserve scheduling error message');
    });

    test('All exceptions extend AnnouncementException', () {
      expect(const NotificationPermissionDeniedException(),
          isA<AnnouncementException>(),
          reason: 'Should extend base exception');
      expect(const NotificationInitializationException('test'),
          isA<AnnouncementException>(),
          reason: 'Should extend base exception');
      expect(const ValidationException('test'), isA<AnnouncementException>(),
          reason: 'Should extend base exception');
      expect(const TTSInitializationException('test'),
          isA<AnnouncementException>(),
          reason: 'Should extend base exception');
      expect(const TTSAnnouncementException('test'),
          isA<AnnouncementException>(),
          reason: 'Should extend base exception');
      expect(const NotificationSchedulingException('test'),
          isA<AnnouncementException>(),
          reason: 'Should extend base exception');
    });
  });
  ...
  group('AnnouncementScheduler API Design', () {
    test('Public API exports are accessible', () {
      ...
      expect(AnnouncementScheduler, isNotNull,
          reason: 'AnnouncementScheduler should be exported');
      expect(AnnouncementConfig, isNotNull,
      ...
      expect(NotificationPermissionDeniedException, isNotNull,
          reason: 'NotificationPermissionDeniedException should be exported');
      expect(NotificationInitializationException, isNotNull,
          reason: 'NotificationInitializationException should be exported');
      expect(NotificationSchedulingException, isNotNull,
          reason: 'NotificationSchedulingException should be exported');
      expect(TTSInitializationException, isNotNull,
          reason: 'TTSInitializationException should be exported');
      expect(TTSAnnouncementException, isNotNull,
